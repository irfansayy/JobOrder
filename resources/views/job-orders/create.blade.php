...
@section('subtitle', 'Add a new job order')
...
<div class="max-w-4xl mx-auto">
    ...
    <form action="{{ route('job-orders.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="order_code" id="order_code" value="{{ $orderCode }}" readonly
                    class="w-full px-4 py-3 border-2 border-slate-200 rounded-lg bg-slate-50 text-slate-600 cursor-not-allowed">
                <p class="text-xs text-slate-500 mt-1">Auto-generated code</p>
            </div>
            ...
            <div>
                ...
                <select name="customer_service_id" id="customer_service_id" required
                    ...
                    <option value="{{ $cs->id }}" {{ old('customer_service_id') == $cs->id ? 'selected' : '' }}>
                        {{ $cs->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="number" name="qty" id="qty" value="{{ old('qty') }}" required min="1"
            ...
            <div>
                ...
                <select name="production_status_id" id="production_status_id" required
                    ...
                    <option value="{{ $status->id }}" {{ old('production_status_id') == $status->id ? 'selected' : '' }}>
                        {{ $status->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="order_priority_id" id="order_priority_id" required
                    ...
                    <option value="{{ $priority->id }}" {{ old('order_priority_id') == $priority->id ? 'selected' : '' }}>
                        {{ $priority->name }}
                    </option>
                    @endforeach
                </select>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label for="po_file" class="block text-sm font-bold text-slate-700 mb-2">
                    <i class="fas fa-file-upload text-teal-600 mr-2"></i>Upload PO File
                </label>
            ...
            <div>
                ...
                <input type="url" name="po_link" id="po_link" value="{{ old('po_link') }}"
        ...
        <div class="flex flex-col sm:flex-row items-center justify-end gap-3 pt-4 border-t border-slate-200">
            ...
            <button type="submit" 
                ...
                <span class="relative"><i class="fas fa-save mr-2"></i>Create Job Order</span>
            </button>
        </div>
    </form>
</div>
