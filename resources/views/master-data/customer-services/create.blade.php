...
@section('subtitle', 'Add a new customer service')

@section('content')
<div class="max-w-2xl mx-auto">
    <form action="{{ route('master-data.customer-services.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <input type="text" name="code" id="code" value="{{ old('code') }}" required
                ...
                placeholder="Enter customer service code (e.g., CS_001)">
            @error('code')
            <p class="text-rose-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4"
                ...
                placeholder="Enter customer service description">{{ old('description') }}</textarea>
            @error('description')
            <p class="text-rose-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex flex-col sm:flex-row items-center justify-end gap-3 pt-4 border-t border-slate-200">
            ...
            <button type="submit" 
                ...
                <span class="relative"><i class="fas fa-save mr-2"></i>Create Customer Service</span>
            </button>
        </div>
    </form>
</div>
