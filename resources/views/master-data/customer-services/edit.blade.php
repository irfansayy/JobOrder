...
@section('subtitle', 'Update customer service information')

@section('content')
<div class="max-w-2xl mx-auto">
    <form action="{{ route('master-data.customer-services.update', $customerService) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')

        <div>
            <label for="name" class="block text-sm font-bold text-slate-700 mb-2">
                <i class="fas fa-user-tie text-teal-600 mr-2"></i>Customer Service Name
            </label>
            <input type="text" name="name" id="name" value="{{ old('name', $customerService->name) }}" required
        ...
        <div>
            <label for="code" class="block text-sm font-bold text-slate-700 mb-2">
                <i class="fas fa-barcode text-teal-600 mr-2"></i>Customer Service Code
            </label>
            <input type="text" name="code" id="code" value="{{ old('code', $customerService->code) }}" required
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4"
                ...
                placeholder="Enter customer service description">{{ old('description', $customerService->description) }}</textarea>
            @error('description')
            <p class="text-rose-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            <label class="flex items-center space-x-3">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $customerService->is_active) ? 'checked' : '' }}
        ...
        <div class="flex flex-col sm:flex-row items-center justify-end gap-3 pt-4 border-t border-slate-200">
            ...
            <button type="submit" 
                ...
                <span class="relative"><i class="fas fa-save mr-2"></i>Update Customer Service</span>
            </button>
        </div>
    </form>
</div>
